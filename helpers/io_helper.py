...
def load_dictionary(filepath, delimiter = '\t', key_int = False):
	lines_split = [x.strip().split(delimiter) for x in load_lines(filepath)]
	keys = [int(x[0]) for x in lines_split if len(x) == 2] if key_int else [x[0] for x in lines_split if len(x) == 2]
	vals = [x[1] for x in lines_split if len(x) == 2]
	print("Size of the loaded dictionary...")	
...
def write_list_tuples_separated_numpyarrays(path, list, delimiter = '\t', append = False):
	f = codecs.open(path,'a' if append else 'w', encoding='utf8')
	for i in range(len(list)):
		for j in range(len(list[i])):
			if j == 0:
				...
				f.write(np.array2string(list[i][j]).replace('\n', '') + '\n')
			else:
				f.write(np.array2string(list[i][j]).replace('\n', '') + delimiter)  
	f.close()
