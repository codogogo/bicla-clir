...
def aggregate_phrase_embedding(words, text_embeddings, stopwords, punctuation, l2_norm_vec = True, lang = 'en', lang_prefix_words = False, min_words = 1):
	word_vecs = []
	...
	if len(word_vecs) >= min_words:
		res = np.sum(word_vecs, axis = 0)
		if l2_norm_vec:
			res = res / np.linalg.norm(res)
		return res
	else:
		#print("No words found in embeddings, returning None")
		return None
	...
	def merge_embedding_spaces(self, languages, emb_size, merge_name = 'merge', lang_prefix_delimiter = '__', special_tokens = None):
		...
		if special_tokens is not None:
			for st in special_tokens:
				merge_vocabulary[st] = len(merge_vocabulary)
				vec = [0.1 * (special_tokens.index(st) + 1)] * emb_size
				merge_norms = np.append(merge_norms, [np.linalg.norm(vec, 2)])
				merge_embs = np.vstack((merge_embs, [vec]))			
