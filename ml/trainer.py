...
class SimpleTrainer(object):
	def __init__(self, model, session, feed_dict_function, eval_func, configuration_func = None, labels = None, additional_results_func = None, model_serialization_path = None):
		self.model = model
		...
		self.labels = labels
		self.model_serialization_path = model_serialization_path
	...
	def cross_validate(self, data, batch_size, max_num_epochs, num_folds = 5, num_devs_not_better_end = 5, batch_dev_perf = 100, print_batch_losses = True, dev_score_maximize = True, configuration = None, print_training = False, micro_performance = True, shuffle_data = True, addit_eval_num_batches = None):
		...
		for i in range(num_folds):
			...
			results[conf_str + "__fold-" + str(i+1)] = self.train_dev(train_data, dev_data, batch_size, max_num_epochs, num_devs_not_better_end, batch_dev_perf, print_batch_losses, dev_score_maximize, configuration, print_training, shuffle_data = shuffle_data, addit_eval_num_batches = addit_eval_num_batches)
		...
		if micro_performance:
			...
			micro_perf = self.eval_func(all_golds, all_preds)
			return results, micro_perf
		else: 
			return results	

	def grid_search(self, configurations, train_data, dev_data, batch_size, max_num_epochs, num_devs_not_better_end = 5, batch_dev_perf = 100, print_batch_losses = True, dev_score_maximize = True, cross_validate = False, cv_folds = None, print_training = False, micro_performance = False, shuffle_data = True, addit_eval_num_batches = None):
		...
		for config in configurations:
			...
			if cross_validate:
				results[str(config)] = self.cross_validate(train_data, batch_size, max_num_epochs, cv_folds, num_devs_not_better_end, batch_dev_perf, print_batch_losses, dev_score_maximize, config, print_training, micro_performance = micro_performance, shuffle_data = shuffle_data, addit_eval_num_batches = addit_eval_num_batches)
			else:
				model, conf_str, session = self.config_func(config)
				self.model = model
				self.session = session
				results[conf_str] = self.train_dev(train_data, dev_data, batch_size, max_num_epochs, num_devs_not_better_end, batch_dev_perf, print_batch_losses, dev_score_maximize, config, print_training, shuffle_data = shuffle_data, addit_eval_num_batches = addit_eval_num_batches)
